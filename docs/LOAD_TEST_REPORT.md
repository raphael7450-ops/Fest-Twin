...
| 시나리오 | 결과 | 핵심 지표 |
|---------|------|----------|
| A. 일반 API 정상 부하 |  PASS | 100/100 성공, TPS 1215.12 |
| B. Rate Limit 방어 |  PASS | 429 응답 35회, 헤더 검증 35회 |
| C. 캐시 성능 |  PASS | 평균 0.74ms (임계값 ≤ 5ms) |
...
|------|-----|
| 대상 엔드포인트 | `/api/tour/area-code` |
| Cold Start (첫 요청) | 2.26ms |
| Cache Hit 측정 횟수 | 19회 |
| Cache Hit 평균 응답 | 0.74ms |
| Cache Hit 최대 응답 | 1.13ms |
| Cache Hit P95 응답 | 1.13ms |
| 임계값 | ≤ 5ms |
| 결과 |  PASS |

> 인메모리 캐시를 통해 동일 요청의 반복 호출 시 네트워크 I/O 없이
> 극도로 빠른 응답(평균 0.74ms)을 달성합니다.
...
| 캐시 응답 속도 |  우수 — 평균 0.74ms |
| TPS (초당 처리량) | 1215.12 req/s |
| 종합 판정 |  ALL PASS |
