...
한국관광공사 OpenAPI 활용 신청 매뉴얼 v3.3 기준으로 운영계정 신청 시 아래 항목을 함께 확인한다.
...
| 사용 오퍼레이션 | `areaCode2`, `searchFestival2`, `detailCommon2`, `locationBasedList2` |
| 호출 흐름 | 지역 선택 → 축제 후보 조회 → 상세 좌표 조회 → 주변 관광지 조회 |
| 개발계정 제한 | 오퍼레이션별 일 1,000건 트래픽 기준으로 검증 |
| 운영계정 승인 | 한국관광공사 담당자 승인 필요, 약 1~3일 소요 |
| 활용 기간 | 운영계정 승인 후 24개월, 만료 시 연장신청 필요 |
| 라이선스/출처 | 한국관광공사 TourAPI 4.0 출처 표기 및 라이선스 표시 동의 전제 |
