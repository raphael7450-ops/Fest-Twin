...
The c028ca3 application deployment returned HTTP 201 for an actual browser-generated snapshot (57,548-byte request). Desktop and 320/390/768px panel checks passed, as did mobile festival switching. Full tests passed with 627 tests and four pre-existing skips; one earlier run encountered an intermittent map timer teardown error, recorded separately from the passing rerun.

Ten browser-generated receipts were preserved across a container restart. The aggregate filename/content SHA-256 was unchanged: `acea1bf0d57489965e0cbf979ca30b9dac8adc10df006508f1ec7db1a77a77de`. The status endpoint reported ten pre-event receipts, zero integrity errors, zero comparable pairs, and two outcomes requiring further review. These are observations at verification time, not permanent counters or a claim of forecasting accuracy.

## September 11 Evidence Update

The [official Gunsan posting](https://www.gunsan.go.kr/main/m82/view/9828373) identifies its publication date as April 15, 2026. The ledger now records that date and an explicitly labeled end-of-day availability bound; it does not invent an exact publication time. This supersedes the earlier unknown-publication-date finding. Counting-method and model-geography compatibility remain unresolved, so `reviewStatus: pending_basis_match` explicitly blocks this outcome even when all publication fields are present. Declared review statuses other than `approved` are excluded by the evaluator and shown in the dashboard.

An [additional official Daejeon release](https://www.daejeon.go.kr/drh/board/boardNormalView.do?boardId=normal_0189&menuSeq=6825&ntatcSeq=1495695140) reiterates the total and mentions AI crowd-density management, but does not establish how the cumulative visitor total was deduplicated. Crowd-density monitoring is not evidence of a unique-visitor counting methodology. Its unknown-method exclusion is retained. No synthetic prediction was added and no coefficient was adjusted.
