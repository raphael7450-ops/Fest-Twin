...
const KNOWN_METRO_REGIONS: Record<string, string[]> = {
  서울: ["서울", "서울특별시", "seoul"],
  부산: ["부산", "부산광역시", "busan"],
  대구: ["대구", "대구광역시", "daegu"],
  인천: ["인천", "인천광역시", "incheon"],
...
function canonicalRegion(value: string | undefined): string {
  if (!value) return "";
...
function regionMatches(planRegion: string, candidateRegion: string): boolean {
  if (!planRegion) return true;
  const canonPlan = canonicalRegion(planRegion);
  const canonCandidate = canonicalRegion(candidateRegion);

  if (canonPlan in KNOWN_METRO_REGIONS) {
    return canonCandidate === canonPlan;
  }

  const reqLower = String(planRegion).replace(/\s+/g, "").toLowerCase();
  const candLower = String(candidateRegion).replace(/\s+/g, "").toLowerCase();
  return candLower.includes(reqLower);
}
...
export const DEFAULT_TOUR_API_AREA_CODES: TourApiAreaCode[] = [
  { code: "1", name: "서울" },
  { code: "2", name: "인천" },
  { code: "3", name: "대전" },
  { code: "4", name: "대구" },
...
async function getSelectedFestivalTourismContext(
  ...
  const isNumericContentId = /^\d+$/.test(String(selectedCandidate.id).trim());
...
export async function getFestivalCandidates(
  ...
  let areaCode: string | number | undefined;
  try {
    areaCode = await resolveAreaCode(plan, fetchImpl, options.signal);
  } catch {
    areaCode = undefined;
  }
  ...
  const regionMatchedItems = mergedFestivalItems.filter(
    (item) =>
  ...
  const overlappingItems = regionMatchedItems.filter((item) =>
